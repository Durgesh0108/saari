...
export default function ZariForm({ categories }) {
  ...
  const onSubmit = async (data: ZariFormValues) => {
    const values = {
      name: data.name,
    ...
    try {
      setLoading(true);
      const response = await axios.post(`/api/zari`, values);
      toggleEdit();
      location.reload();
      toast.success("Zari Created Successfully");
    } catch (error: any) {
      console.log(error);
