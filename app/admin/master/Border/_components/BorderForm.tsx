...
type BorderFormValues = z.infer<typeof formSchema>;

export default function BorderForm({ categories }) {
  ...
  const onSubmit = async (data: BorderFormValues) => {
    const values = {
      name: data.name,
    ...
    try {
      setLoading(true);
      const response = await axios.post(`/api/border`, values);
      toggleEdit();
      location.reload();
      toast.success("Border Created Successfully");
    } catch (error: any) {
      console.log(error);
